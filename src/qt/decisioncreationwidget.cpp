...
#include "forms/ui_decisioncreationwidget.h"
...
json_spirit::Array DecisionCreationWidget::createDecisionArray()
{
    ...
    if (branchID.isEmpty()) {
        Q_EMIT inputError("You must select a branch!");
        error = true;
    }
    ...
    if (prompt.isEmpty()) {
        Q_EMIT inputError("You must enter a valid prompt!");
        error = true;
    }

    if (eventOverBy == 0) {
        Q_EMIT inputError("You must enter a valid ending date!");
        error = true;
    }

    if (scaled && (scaledMax == scaledMin)) {
        Q_EMIT inputError("You must enter  valid scaled values!");
        error = true;
    }
...
void DecisionCreationWidget::on_pushButtonCreateDecision_clicked()
{
    ...
    if (!decisionArray.empty()) {
        Q_EMIT newDecisionArray(decisionArray);
    }
}
...
void DecisionCreationWidget::on_pushButtonUpdateDecision_clicked()
{
    ...
    if (!updatedDecision.empty() && updateIndex >= 0) {
        ...
        Q_EMIT updatedDecisionArray(updatedDecision);
    }
}
