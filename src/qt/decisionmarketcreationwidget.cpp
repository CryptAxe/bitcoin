...
#include "forms/ui_decisionmarketcreationwidget.h"
...
json_spirit::Array DecisionMarketCreationWidget::createDecisionMarketArray()
{
    ...
    if (address.isEmpty()) {
        Q_EMIT inputError("You must enter a valid Hivemind address!");
        error = true;
    }

    if (decisions.size() == 0) {
        Q_EMIT inputError("You must enter a decision ID!");
        error = true;
    }

    if (B == 0) {
        Q_EMIT inputError("You must enter a valid B value!");
        error = true;
    }

    if (tradingFee == 0) {
        Q_EMIT inputError("You must enter a valid trading fee!");
        error = true;
    }

    if (maxCommission == 0) {
        Q_EMIT inputError("You must enter a valid max commission!");
        error = true;
    }

    if (title.isEmpty()) {
        Q_EMIT inputError("You must enter a valid title!");
        error = true;
    }

    if (description.isEmpty()) {
        Q_EMIT inputError("You must enter a valid Hivemind description!");
        error = true;
    }

    if (tags.isEmpty()) {
        Q_EMIT inputError("You must enter valid tags!");
        error = true;
    }

    if (maturation == 0) {
        Q_EMIT inputError("You must enter a valid maturation date!");
        error = true;
    }

    if (txPoWd == 0) {
        Q_EMIT inputError("You must enter a valid PoW difficulty!");
        error = true;
    }
...
void DecisionMarketCreationWidget::on_pushButtonCreateMarket_clicked()
{
    ...
    if (!decisionMarketArray.empty()) {
        Q_EMIT newDecisionMarketArray(decisionMarketArray);
    }
}
...
void DecisionMarketCreationWidget::on_pushButtonUpdateMarket_clicked()
{
    ...
    if (!updatedMarket.empty() && updateIndex >= 0) {
        ...
        Q_EMIT updatedDecisionMarketArray(updatedMarket);
    }
}
