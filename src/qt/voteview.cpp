...
void VoteView::on_pushButtonCreateRevealVote_clicked()
{
    ...
    if (!branch) {
        Q_EMIT displayMessage("Error creating Revealed Vote", "Invalid Branch ID, branch does not exist!");
        return;
    }

    std::string strAddress = ui->lineEditVotecoinAddress->text().toStdString();
    if (!IsValidDestinationString(strAddress)) {
        Q_EMIT displayMessage("Error creating Reveald Vote", "Invalid Votecoin address!");
        return;
    }

    // Get the voter's address
    CTxDestination voterAddress = DecodeDestination(strAddress);
    // TODO
    ...
    if (!sealedVote) {
        Q_EMIT displayMessage("Error creating Revealed Vote", "Vote does not exist!");
        return;
    }

    // Create spirit array of sealed vote params
    json_spirit::Array revealVoteParams;
    revealVoteParams.push_back(strAddress);
    ...
    try {
        ...
        Q_EMIT displayMessage("Error creating Revealed Vote", errorText);
        return;
    } catch (const std::exception &exception) {
        QString exceptionText = QString::fromStdString(exception.what());
        Q_EMIT displayMessage("Error creating Revealed Vote", exceptionText);
        return;
    ...
    try {
        ...
        Q_EMIT displayMessage("Revealed Vote Submitted!", resultMessage);

    } catch (const std::runtime_error &error) {
        Q_EMIT displayMessage("Error creating Revealed Vote!", QString::fromStdString(error.what()));

    } catch (const std::exception &exception) {
        Q_EMIT displayMessage("Error creating Revealed Vote!", QString::fromStdString(exception.what()));
    }
}
...
void VoteView::on_pushButtonCreateSealedVote_clicked()
{
    ...
    marketBranch *branch = pmarkettree->GetBranch(uBranch);
    if (!branch) {
        Q_EMIT displayMessage("Error creating Sealed Vote", "Branch does not exist!");
        return;
    }
    ...
    if (!decision) {
        Q_EMIT displayMessage("Error creating Sealed Vote", "Decision does not exist!");
        return;
    }
    ...
    revealVote.NA = 2016;
    // TODO
    // This can't seriously be how we have to do this now???
    // Core has deleted the CBitcoinAddress class, and I haven't
    // figured out the proper way to extract the keyID yet...
    // Obviously this is really stupid though and either we need
    // to figure out the proper new way, or replace the much
    // more simple code that core removed.
    revealVote.keyID = boost::get<CKeyID>(voterAddress);
    ...
    try {
        ...
        Q_EMIT displayMessage("Error creating Sealed Vote", QString::fromStdString(error.what()));
        return;
    } catch (const std::exception &exception) {
        Q_EMIT displayMessage("Error creating Sealed Vote", QString::fromStdString(exception.what()));
        return;
    } catch (const json_spirit::Object &object) {
    ...
    try {
        ...
        if (codePair.name_ == "code") {
            int code = codePair.value_.get_int();
            if (code < 0) {
                Q_EMIT displayMessage("Error creating Sealed Vote", QString::fromStdString(messagePair.value_.get_str()));
                return;
            }
        }
        ...
        resultMessage.append(":\n");
        resultMessage.append(QString::fromStdString(messagePair.value_.get_str()));
        Q_EMIT displayMessage("Sealed Vote Submitted!", resultMessage);
